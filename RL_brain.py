...
class DeepQNetwork:
    ...
    def _build_net(self):
        ...
        self.q_target = tf.placeholder(tf.float32, [None, self.num_actions], name="Q_target") # for calculating loss like y in other network
        with tf.variable_scope("predict_net"):
            ...
            with tf.variable_scope("output"):
                ...
                self.q_predict = tf.matmul(l2,w3) + b3
        ...
        with tf.variable_scope("target_net"):
            ...
            with tf.variable_scope("output"):
                w3 = tf.get_variable('w3', [num_layer_2, self.num_actions], initializer=w_initializer,
                                     collections=c_names)
                b3 = tf.get_variable('b3', [1, self.num_actions], initializer=b_initializer, collections=c_names)
                self.q_predict_next = tf.matmul(l2, w3) + b3
    ...
    def learn(self):
        ...
        q_predict_next, q_predict = self.sess.run(
            [self.q_predict_next, self.q_predict],
        ...
        q_target[batch_index, predict_act_index] = reward + self.reward_decay * np.max(q_predict_next, axis = 1)
        # print("q_target.shape:",q_target.shape)
        # train predict network
        _, self.cost = self.sess.run([self._train_op,self.loss],
                                     feed_dict={self.s: batch_memory[:, :self.num_features],self.q_target: q_target})
